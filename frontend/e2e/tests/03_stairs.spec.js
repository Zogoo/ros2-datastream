...
// Resting deck: 45 mm step at x=-6.24..-5.68 then the 90 mm deck behind it.
test('stairs: suspension climbs the resting-deck steps without e-stop', async ({ page }) => {
  ...
  await page.evaluate(() => window.__sim.setPose(-4.16, 2.08, Math.PI));
  ...
  let pose = before;
  for (let i = 0; i < 40 && pose.x > -6.48; i++) {
    await sleep(200);
  ...
  expect(after.x, 'front wheels must be up on the deck').toBeLessThan(-6.32);
